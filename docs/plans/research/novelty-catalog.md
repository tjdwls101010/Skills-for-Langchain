...
| `RubricMiddleware` (Beta) | NOVEL | `deepagents>=0.6.5`. LLM-as-judge self-grading loop. Args `model`, `system_prompt`, `tools`, `max_iterations`(3), `on_evaluation`. Pass `rubric` string at invoke. Verdicts: satisfied/needs_revision/max_iterations_reached/failed/grader_error. Emits `rubric_evaluation_start/end` on `stream.custom`. Also `RubricCodeGenerationMiddleware`. | rubric, code/goals-and-rubrics |
